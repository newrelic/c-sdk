...
function test_prepare_named($conn)
{
  $query = "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_NAME = 'pg_roles'";

  if (!pg_prepare($conn, 'my query', $query)) {
      echo pg_last_error($conn) . "\n";
      return;
  }

  $result = pg_execute($conn, 'my query', array());
  if (!$result) {
      echo pg_last_error($conn) . "\n";
      return;
  }
...
test_prepare_unnamed($conn);
test_prepare_named($conn);
pg_close($conn);
