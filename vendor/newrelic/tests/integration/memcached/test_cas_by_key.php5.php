...
function main()
{
  ...
  $value = $test->getByKey($server_key, $key, NULL, $token);
  tap_equal('hot potato', $value, 'getByKey');

  $test->casByKey($token, $server_key, $key, 'cold potato');

  $value = $test->getByKey($server_key, $key, NULL, $token);
  tap_equal('cold potato', $value, 'getByKey');
