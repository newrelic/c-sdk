...
function main()
{
  ...
  $values = $test->get($key, NULL, Memcached::GET_EXTENDED);
  tap_equal('hot potato', $values['value'], 'get value');

  $test->cas($values['cas'], $key, 'cold potato');

  $values = $test->get($key, NULL, Memcached::GET_EXTENDED);
  tap_equal('cold potato', $values['value'], 'get value');
  $test->delete($key);
