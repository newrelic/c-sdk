...
func Hostname() (string, error) {
	// Try the builtin API first, which is designed to match the output
	// of /bin/hostname, and fallback to uname(2) if that fails to match
	// the behavior of gethostname(2) as implemented by glibc. On Linux, all
	...
	// the current processes' filesystem namespace via the cgroups APIs. Docker
	// is an example of a tool that can configure such an environment.
	...
	if err2 := syscall.Uname(&uts); err2 != nil {
		// The man page documents only one possible error for uname(2), suggesting
		// that as long as the buffer given is valid, the call will never fail.
		// Return the original error in the hope it provides more relevant
		// information about why the hostname can't be retreived.
		return "", err
	}
