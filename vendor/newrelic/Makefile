...
go-minimum-version:
	@if $(GO) version | awk '/go1.[0123456][\. ]/ { exit 1 }'; then \
		true; \
	else \
		echo -n 'Go 1.7 or newer required; found '; $(GO) version; false; \
	fi
...
.PHONY: daemon_race
daemon_race: go-minimum-version Makefile | bin/
	@rm -rf $(DAEMON_TARGETS)
